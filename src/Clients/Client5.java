...
/**
 *This is client number 5,the code for client 1 and 2 seems to be the same and could have been implemented as just
...
public class Client5
{
    ...
    public static void sendMessages() throws IOException {
        ...
        tempSocket1.close();
        System.out.println("client5:" + localHost)  ;
        Peer peer = new Peer(localHost, 49159);
        while(true)
        {
            ...
            if(userInput.equals("STARTUP")) {
                LinkedList<RFC> RFCList = ClientHelper.getRFCList("src\\ClientFiles\\Client5");
                for (RFC rfc : RFCList) {
                    out.flush();
                ...
                if(!ClientHelper.getMethod(lookupInput).equals("200 OK")) {
                    ...
                    StringBuffer sb = new StringBuffer("src\\ClientFiles\\Client5\\rfc");
                    sb.append(rfcID);
                    sb.append(".txt");
    ...
    public static void main(String [] args) throws IOException {
        new receieveMessage5().start();
        sendMessages();
    }
}


class receieveMessage5 extends Thread{
    ...
    public void run(){
        try {
            ServerSocket serverSocket = new ServerSocket(49159);
            while(true) {
                ...
                System.out.println("Inside Client 5 "+ new String(buffer));
                Integer rfcID =ClientHelper.getRFCIDFromGETMessage(new String(buffer));
                file= new File("src\\ClientFiles\\Client5\\rfc"+rfcID+".txt");
                fileReader = new BufferedInputStream(new FileInputStream(file));
                StringBuffer sb = new StringBuffer();
                sb.append("src\\ClientFiles\\Client5\\rfc");
                sb.append(rfcID);
                sb.append(".txt");
